class ClientSubscriptionsManager {
    constructor() {
        ...
        this.mySubscriptions = []; // Все активные абонементы
    }
    ...
    // Загрузить информацию о профиле клиента
    loadClientProfile() {
    ...
    // Загрузить доступные абонементы для покупки
    ...
    // Загрузить активные абонементы клиента
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    // Если маршрут не найден, показываем сообщение об отсутствии абонементов
            ...
            if (data.success) {
                ...
                this.mySubscription = data.subscription; // Для обратной совместимости
            ...
            // В случае ошибки показываем пустое состояние вместо ошибки
            this.mySubscriptions = [];
            this.mySubscription = null;
            this.renderMySubscription([]);
        }
    ...
    // Отобразить доступные абонементы для покупки
    renderAvailableSubscriptions(subscriptions) {
        ...
        container.innerHTML = subscriptions.map(subscription => {
            ...
            // Проверяем, можно ли купить этот абонемент
            ...
            // Нельзя купить, если уже есть такой же тип (кроме combo, который заменяет все)
            // Combo можно купить всегда (он заменит все)
            // Group и Gym можно купить, если нет такого же типа
            let canPurchase = true;
            let disabledReason = '';
            
            if (subscriptionType === 'combo') {
                // Combo можно купить всегда
                canPurchase = true;
            } else if (hasSameType) {
                canPurchase = false;
                disabledReason = 'У вас уже есть абонемент этого типа';
            } else if (hasCombo) {
                // Если есть combo, можно купить group или gym (combo будет заменен)
                canPurchase = true;
            }
    ...
    // Отобразить активные абонементы клиента
    renderMySubscription(subscriptions) {
        ...
        // Отображаем все активные абонементы
        container.innerHTML = subscriptions.map(subscription => {
    ...
    async purchaseSubscription(subscriptionTypeId) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    try {
                        ...
                        // Если не JSON, это HTML страница ошибки
            ...
            if (data.success) {
                ...
                // Обновляем список активных абонементов и доступных для покупки
                await this.loadMySubscription();
                await this.loadAvailableSubscriptions();
            } else {
