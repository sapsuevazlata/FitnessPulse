class ClientTrainingsManager {
    constructor() {
        ...
        this.selectedInventory = []; // Массив выбранных ID инвентаря
        this.inventoryList = []; // Список всего инвентаря из БД
        this.init();
    }

    init() {
        // Устанавливаем минимальную дату (сегодня) и ограничения времени
        ...
        // Устанавливаем ограничения времени для тренировки без тренера (9:00-21:00)
    ...
    // Открыть модальное окно выбора типа тренировки
    openBookingModal() {
    ...
    // Открыть модальное окно записи без тренера
    ...
    async loadTrainers() {
        try {
            // Используем тот же endpoint, что и в каталоге
    ...
    // Загрузить расписание тренера и открыть модальное окно
    async loadTrainerScheduleAndOpenModal() {
        if (!this.selectedTrainer) return;

        try {
            // Загружаем расписание тренера через публичный API
            ...
            if (response.ok) {
                ...
                if (data.success && data.schedule && data.schedule.length > 0) {
                    // Сохраняем расписание для использования в форме
    ...
    // Заполнить select расписанием тренера
    populateScheduleSelect(schedule) {
        ...
        // Добавляем обработчик изменения для загрузки доступных дат
        ...
        if (updatedSelect) {
            ...
            // Автоматически выбираем первый вариант и загружаем даты
    ...
    // Заполнить select доступными датами для выбранного дня недели
    populateDateSelect(dayOfWeek) {
        ...
        // Генерируем список доступных дат (ближайшие 4 недели)
        ...
        dates.forEach(date => {
            // Парсим дату в формате YYYY-MM-DD (локальное время, без учета часового пояса)
            const [year, month, day] = date.split('-').map(Number);
            const dateObj = new Date(year, month - 1, day, 12, 0, 0); // Используем полдень, чтобы избежать проблем с часовыми поясами
            ...
            // Проверяем, что день недели соответствует выбранному
        ...
        // Автоматически выбираем первую дату
    ...
    // Получить доступные даты для дня недели
    getAvailableDatesForDayOfWeek(dayOfWeek, weeksAhead = 4) {
        ...
        if (daysUntilTarget < 0) {
            ...
            // Если сегодня этот день, всегда берем следующий раз
            // чтобы избежать проблем с записью на сегодня, если время уже прошло
            daysUntilTarget = 7;
        }
    ...
    // Открыть модальное окно записи с тренером
    ...
    async loadPaymentMethods(type) {
        try {
            ...
            // Находим активный абонемент типа gym или combo
            ...
            // Для тренировки с тренером абонемент не показываем
            if (type === 'with-trainer') {
                ...
                // Для тренировки без тренера показываем все способы оплаты
                container.innerHTML = `
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                        <input type="radio" name="${paymentName}" value="qr_code" checked onchange="clientTrainingsManager.toggleQRCode('${type}', 'qr_code')">
                        <span>QR-код для оплаты</span>
            ...
            // Если не удалось загрузить, показываем только QR-код и оплату на месте
            const container = type === 'no-trainer' 
                ? document.getElementById('payment-method-no-trainer')
                : document.getElementById('payment-method-with-trainer');
            if (container) {
                ...
                if (type === 'with-trainer') {
                    ...
                    // Для тренировки без тренера показываем QR и оплату на месте
                    container.innerHTML = `
                        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                            <input type="radio" name="${paymentName}" value="qr_code" checked onchange="clientTrainingsManager.toggleQRCode('${type}', 'qr_code')">
                            <span>QR-код для оплаты</span>
    ...
    toggleQRCode(type, paymentMethod) {
        ...
        if (qrContainer) {
            if (paymentMethod === 'qr_code') {
                ...
                // Генерируем случайный код для QR
    ...
    // Отправить форму записи без тренера
    async submitWithoutTrainerForm(e) {
        ...
        // Проверка времени для тренировки без тренера (9:00-21:00)
        if (time) {
        ...
        // Используем выбранный инвентарь из модального окна
        const selectedInventoryNames = this.selectedInventory.map(id => {
            const item = this.inventoryList.find(inv => inv.id === parseInt(id));
        ...
        const bookingData = {
            ...
            equipment: selectedInventoryNames.length > 0 ? selectedInventoryNames.join(', ') : null
        };
        ...
        try {
            ...
            if (data.success) {
                ...
                if (selectedInventoryNames.length > 0) {
                    const equipmentList = selectedInventoryNames.join(', ');
                    message += ` Забронирован инвентарь: ${equipmentList}.`;
                }
                this.showNotification(message, 'success');
                this.closeModals();
                form.reset();
                this.selectedInventory = []; // Очищаем выбранный инвентарь
    ...
    // Отправить форму записи с тренером
    async submitWithTrainerForm(e) {
        ...
        // Парсим day_of_week и time из значения select
        ...
        // Получаем выбранную дату из select
        const dateSelect = document.getElementById('training-date-select-with-trainer');
        ...
        const bookingData = {
            ...
            booking_time: time,
            day_of_week: dayOfWeek, // Отправляем также день недели для проверки на backend
            payment_method: paymentMethod,
            subscription_id: subscriptionId || null,
            equipment: selectedInventoryNames.length > 0 ? selectedInventoryNames.join(', ') : null
        };
    ...
    // Получить следующую дату для указанного дня недели
    getNextDateForDayOfWeek(dayOfWeek) {
        ...
        if (daysUntilTarget < 0) {
            ...
            // Если уже поздно, берем следующий раз
            const now = new Date();
            const hours = now.getHours();
            if (hours >= 20) { // Если уже после 20:00, берем следующий раз
                daysUntilTarget = 7;
            }
        }
    ...
    // Открыть модальное окно для бронирования инвентаря
    ...
    renderInventoryList() {
        ...
        container.innerHTML = this.inventoryList.map(item => {
            const isSelected = this.selectedInventory.includes(item.id.toString());
            return `
                <label style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; border: 1px solid ${isSelected ? 'var(--primary)' : 'rgba(255,255,255,0.1)'}; border-radius: 8px; cursor: pointer; background: ${isSelected ? 'rgba(74, 144, 226, 0.1)' : 'transparent'}; transition: all 0.2s;">
                    <input type="checkbox" 
                           value="${item.id}" 
                           ${isSelected ? 'checked' : ''} 
                           onchange="clientTrainingsManager.toggleInventory(${item.id})"
    ...
    toggleInventory(inventoryId) {
        const idStr = inventoryId.toString();
        const index = this.selectedInventory.indexOf(idStr);
        
        if (index > -1) {
            this.selectedInventory.splice(index, 1);
        } else {
            this.selectedInventory.push(idStr);
        }
    ...
    // Обновить отображение выбранного инвентаря в форме
    updateSelectedInventoryDisplay() {
        ...
        const selectedNames = this.selectedInventory.map(id => {
            const item = this.inventoryList.find(inv => inv.id === parseInt(id));
            return item ? item.name : null;
        }).filter(name => name !== null);
        ...
        listDiv.innerHTML = selectedNames.map(name => 
            `<span style="display: inline-block; padding: 0.25rem 0.5rem; margin: 0.25rem; background: var(--primary); color: white; border-radius: 4px; font-size: 0.85rem;">${name}</span>`
        ).join('');
        displayDiv.style.display = 'block';
    }

    // Подтвердить выбор инвентаря
    confirmInventorySelection() {
        this.closeInventoryModal();
    }

    // Закрыть модальное окно инвентаря
    closeInventoryModal() {
        const modal = document.getElementById('inventory-modal');
        if (modal) {
            modal.style.display = 'none';
...
function initClientTrainingsManager() {
    ...
    window.clientTrainingsManager = clientTrainingsManager; // Делаем глобально доступным
